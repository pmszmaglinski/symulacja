...
#include <counter.h> // Dodana klasa licznika
...
class BaseWaveApplLayer : public BaseApplLayer {
	...
	protected:
		...
		virtual void onVoIP(WaveShortMessage* wsm) = 0; // Dodana deklaracja
	...
	protected:
		...
		bool sendData;
		bool sendBeacons;
		bool sendVoIP;  // Dodana zmienna określająca czy wysyłać VoIP
		simtime_t individualOffset;
		int dataLengthBits;
		int voipLengthBits; // Dodana zmienna - długość pakietu VoIP
		bool dataOnSch;
		bool voipOnSch; // Dodana zmienna - kanał dla VoIP
		int dataPriority;
		int voipPriority;   // Dodana zmienna - priorytet dla wiadomości VoIP
		Coord curPosition;
		int mySCH;
		int myId;
		cMessage* sendBeaconEvt;
		cMessage* sendVoIPEvt; //Dodanie deklaracji zmiennej wiadomości VoIP do pakietyzacji
